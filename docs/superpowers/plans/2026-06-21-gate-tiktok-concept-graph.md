...
### Task 11: No-scroll viewport pagination
...
- [ ] Add failing static tests prohibiting nested `overflow-y:auto`, `scrollbar-width:thin`, and scrollable card interiors while allowing the hidden-scroll `.gt-feed`.
- [ ] Add failing runtime markers for card-page and concept-page Previous/Next controls, page indicators, and page-state restoration.
- [ ] Run `node --test tests/gate-tiktok.test.js` and verify the existing question, definition, concept, graph, and node-panel scroll rules fail.
- [ ] Add feed-card pagination helpers that partition definition content and question content into viewport-sized semantic pages. Keep answers and controls together and never split a formula from its immediate prompt.
- [ ] Render card pagination controls only when a card has multiple pages. Stop propagation on page controls and preserve the feed card index.
- [ ] Replace concept-sheet continuous markup with semantic concept pages: meaning, examples, consequences, GATE, connections, and actions.
- [ ] Store concept page index in concept history and graph-entry state. Returning from graph mode restores the original concept page, not a pixel scroll offset.
- [ ] Fit graph canvas and selected-node panel into fixed grid rows. Limit the accessible relationship list to the strongest visible relationships without scrolling.
- [ ] Remove nested overflow rules from question cards, definition cards, concept content, graph panels, graph-node panels, formulas, tables, and dialogs. Hide the feed scrollbar while retaining swipe/snap navigation.
- [ ] Use viewport-height media queries to reduce type, spacing, and graph height on short screens without dropping required controls.
- [ ] Run automated tests and browser assertions that `scrollHeight <= clientHeight + 1` for representative definition, MCQ/MSQ/NAT, concept, graph, and graph-node views at 360×640, 390×844, 768×1024, and 1440×900.
- [ ] Commit with `fix: remove nested Gate TikTok scrolling`.
